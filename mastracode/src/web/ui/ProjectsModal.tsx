...
export function ProjectsModal({
  ...
  const handleGitSubmit = (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      const project = addGitProject(gitUrl, railwayEnabled ? undefined : cloneParentPath);
      onProjectsChange(loadProjects());
      onSelectProject(project);
      onClose();
    } catch (err) {
                ...
                <form className="projects-git-form" onSubmit={handleGitSubmit}>
                  ...
                  {railwayEnabled ? (
                    <p className="projects-git-hint">
                      The repository will be cloned into the Railway sandbox at <code>/workspace</code> and reused for
                      future sessions.
                    </p>
                  ) : (
                    <>
                  ...
                  <div className="projects-git-actions">
                    ...
                    <button
                      ...
                      type="submit"
                      disabled={busy || !gitUrl.trim() || (!railwayEnabled && !cloneParentPath.trim())}
                    >
                      Clone from URL
                    </button>
                  </div>
